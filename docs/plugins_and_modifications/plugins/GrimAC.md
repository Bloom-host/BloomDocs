...
While we usually recommend that you install ViaVersion on your proxy, GrimAC requires ViaVersion to be installed on the backend servers where GrimAC is installed. ViaVersion must not be installed on the proxy, when using GrimAC. Other Via plugins should also be installed on the backend.
:::
...
Besides being a standalone Anticheat, GrimAC is also commonly used along with [Vulcan Anticheat](/plugins/vulcan) for a more comprehensive approach to hacking prevention and punishment.
...
- Network Auditing: GrimAC is especially good at compensating for latency and other packet tweaking methods due to its ability to predict what a right packet (and order) should look like.
...
- [ ] GrimAC has [PacketEvents](https://modrinth.com/plugin/packetevents) as a dependency. However, GrimAC now includes PacketEvents by default. You might need to add PacketEvents if you try to use older versions.
...
In the `GrimAC` folder within `plugins` we'll find 2 main config files which are of interest to us, and a directory:

1. **config.yml**: Here we'll find the bulk of the configuration options we care about. Mainly, it includes everything detection related, meaning the triggers that cause GrimAC to act.
...
First we'll deal with `database.yml`. This is pretty simple, the development team recommends that anyone who is not running a network stay on the default, `SQLite`. However, if you are running a network (with multiple backends) or plan to do so eventually, you should use `mysql`. To start using MariaDB/MySQL, you need to change the first part of `database.yml` to look like the example below:
...
Now, you'll need to create a new database following our [Database Guide](/databases). After creating a database, you'll need to edit the `mysql.yml` file which we'll find within the `databases` directory. You'll only need to edit the credentials so they look like the example below, using your own credentials as seen in the guide linked previously:
...
The GrimAC config is pretty straightforward. If you ever have issues with a specific check, you can try to either lower or increase the detection value (depending on if you get false positives or false negatives) to your liking.

If you have issues with too many misses or too many undue kicks, we recommend contacting the GrimAC community over on their [Support Discord](https://discord.com/invite/kqQAhTmkUF).
