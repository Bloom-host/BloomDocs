...
![Server Splitter form configured for a Velocity proxy](/running_a_server/velocity/velocity-server-split-setups.png)
...
![DuckPanel showing the newly created Velocity server split](/running_a_server/velocity/2.png)
...
![Velocity server Startup tab with the server jar variable](/running_a_server/velocity/velocity-startup-options.PNG)

Now you need to start up the Velocity server and then shut it down once it's loaded, this will initialize the filesystem for Velocity and set up Velocity's folders and configuration file. Your `/home` directory should now look something like this.
...
![spigot.yml with BungeeCord forwarding disabled](/running_a_server/velocity/4.png)
...
![Velocity proxy forwarding.secret file](/running_a_server/velocity/5.png)

![Forwarding secret copied from the Velocity proxy](/running_a_server/velocity/6.png)
...
![paper-global.yml with the Velocity forwarding secret configured](/running_a_server/velocity/7.png)
...
:::caution
Legacy BungeeCord forwarding mode is less secure than modern Velocity forwarding mode and may result in security issues if not set up properly.
...
| -------- | --------- | ---------- |
| **[forced-hosts]**  | This lets you set up your custom domain with Velocity; set the right side to match your **try** section from earlier | `"host.name" = ["lobby", "survival", "creative"]` |
...
If you are interested in installing additional functionality for Velocity, browse the [Velocity plugins forum](https://forums.velocitypowered.com/c/plugins/5) for compatible plugins!
