...
description: Create, import, and manage MySQL databases for plugins in DuckPanel.
keywords:
  - Databases
  - MySQL
  - SQL
...
### Database Storage
The data stored by these databases do not affect your server's storage limit. However, you should follow our [Fair Usage Policy](https://bloom.host/terms/#fair-usage-policy-for-bloom-minecraft-and-game-hosting-plans) regarding database usage.
> This does not apply to dedicated servers, and MySQL databases will contribute to the dedicated server's overall data usage.

<div style={{ position: 'relative', paddingTop: '56.25%', textAlign: 'center' }}>
  <iframe
    src="https://www.youtube.com/embed/X_JCB6NbIEI?si=NHmEau_Za8Yy2_hS"
...
You can also upload an SQL file to import a database. Once the file is uploaded, go to the file manager, click the three dots to the right of the .sql file and click on **Import Database** then select the database you want to import it to. Keep in mind that not all .sql files are in the correct format and this option may not work if it wasn't a database properly dumped from a MySQL server.
